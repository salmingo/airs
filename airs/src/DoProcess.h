...
protected:
	/* 数据类型 */
	typedef boost::container::deque<string> strque;
	...
	typedef boost::container::stable_vector<AstroDIPtr> AstroDIPVec;
	typedef boost::container::stable_vector<AstroMetryPtr> AstroMetryVec;
	typedef boost::container::stable_vector<PhotoMetryPtr> PhotoMetryVec;
...
protected:
	 ...
	 * -3: 流量定标流程失败
	 */
	void on_complete_image(const long rslt, const long);
	/*!
	 * @brief 响应消息MSG_START_ASTROMETRY
	 */
	void on_start_astrometry(const long rslt, const long);
	/*!
	 * @brief 响应消息MSG_START_PHOTOMETRY
	 */
	void on_start_photometry(const long rslt, const long);
};
